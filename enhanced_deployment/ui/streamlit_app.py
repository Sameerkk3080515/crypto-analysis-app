...
from data_collection.real_time_data import RealTimeDataManager
...
def create_binance_logo():
    # Create a simple Binance-inspired logo
    fig, ax = plt.subplots(figsize=(3, 1.5))
    ax.set_facecolor('#1E2026')
    fig.patch.set_facecolor('#1E2026')
    
    # Draw the Binance logo (simplified version)
    ax.text(0.5, 0.5, 'B', fontsize=50, fontweight='bold', 
            color='#F0B90B', ha='center', va='center')
    
    # Add text
    ax.text(0.5, 0.2, 'SPOT TRADING ASSISTANT', fontsize=10, 
...
# Function to get cryptocurrency icon
def get_crypto_icon(symbol):
    # Extract base symbol (e.g., BTC from BTCUSDT)
    base_symbol = symbol.replace('USDT', '')
    
    # Create a colored circle with the symbol text
    fig, ax = plt.subplots(figsize=(1, 1))
    
    # Use a hash of the symbol to generate a consistent color
    color_hash = hash(base_symbol) % 0xFFFFFF
    ...
    # Add symbol text
    ax.text(0.5, 0.5, base_symbol[:4], fontsize=14, fontweight='bold', 
            color='white', ha='center', va='center')
...
def display_welcome():
    # Display logo
    logo_buf = create_binance_logo()
    cols = st.columns([1, 2, 1])
    with cols[1]:
        st.image(logo_buf, use_column_width=True)
...
def run_analysis():
    global data_manager, last_refresh_time
    ...
    if last_refresh_time is None or (current_time - last_refresh_time).total_seconds() > 300:
...
def display_dashboard():
    ...
    logo_buf = create_binance_logo()
    st.sidebar.image(logo_buf, use_column_width=True)
    ...
    st.markdown(
        ...
        <div style="background-color: #474D57; padding: 20px; border-radius: 10px; margin-bottom: 20px; border-left: 5px solid {action_color};">
            ...
            <div style="display: flex; justify-content: space-between; margin-bottom: 15px;">
                <div>
                    <p><strong>Current Price:</strong> ${current_price:.6f if current_price < 0.01 else current_price:.4f if current_price < 1 else current_price:.2f}</p>
        ...
        with st.expander(f"{rec['symbol']} - Confidence: {rec['confidence_score']:.1f}/100", expanded=False):
            ...
            with col1:
                ...
                if rec['symbol'] in all_pairs_data and 'klines' in all_pairs_data[rec['symbol']]:
                    ...
                    projected_data = data_manager.generate_projected_data(historical_data, rec['symbol'])
            ...
            with col2:
                ...
                st.markdown(
                    ...
                    <div style="background-color: #2c3e50; padding: 10px; border-radius: 5px; margin-bottom: 10px;">
                        <p><strong>Current Price:</strong> ${rec['current_price']:.6f if rec['current_price'] < 0.01 else rec['current_price']:.4f if rec['current_price'] < 1 else rec['current_price']:.2f}</p>
            ...
            st.markdown(
                ...
                <div style="background-color: #2c3e50; padding: 10px; border-radius: 5px; display: flex; justify-content: space-between;">
                    <div>
                        <p><strong>Support:</strong> ${rec['closest_support']:.6f if rec['closest_support'] < 0.01 else rec['closest_support']:.4f if rec['closest_support'] < 1 else rec['closest_support']:.2f}</p>
                    </div>
                    <div>
                        <p><strong>Current:</strong> ${rec['current_price']:.6f if rec['current_price'] < 0.01 else rec['current_price']:.4f if rec['current_price'] < 1 else rec['current_price']:.2f}</p>
                    </div>
                    <div>
                        <p><strong>Resistance:</strong> ${rec['closest_resistance']:.6f if rec['closest_resistance'] < 0.01 else rec['closest_resistance']:.4f if rec['closest_resistance'] < 1 else rec['closest_resistance']:.2f}</p>
                    </div>
                </div>
                """,
                unsafe_allow_html=True
